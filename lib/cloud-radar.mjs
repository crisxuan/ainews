...
export function mergeDuplicateBreakingItems(rows) {
  if (!Array.isArray(rows) || rows.length === 0) return null;
  const sorted = [...rows].sort(
    (a, b) => new Date(b.detected_at).getTime() - new Date(a.detected_at).getTime(),
  );
  const keeper = sorted[0];
  const sources = uniqueStrings(sorted.flatMap((row) => row.sources ?? []), 8);
  const discussions = new Map();
  for (const link of sorted.flatMap((row) => row.discussion_links ?? [])) {
    const url = normalizeUrl(link?.url);
    if (url && !discussions.has(url)) discussions.set(url, { ...link, url });
  }
  return {
    keeperId: keeper.id,
    removeIds: sorted.slice(1).map((row) => row.id),
    sources,
    discussionLinks: [...discussions.values()].slice(0, 4),
    sourceCount: Math.max(
      sources.length,
      ...sorted.map((row) => Number(row.source_count ?? 0)),
    ),
  };
}

export function canonicalEvidenceLinks(topic) {
  return uniqueStrings(
    [topic?.link, ...(topic?.evidenceLinks ?? [])]
      .map(normalizeUrl)
      .filter(isAllowedUrl),
    12,
  );
    ...
    return [
      {
        ...
        sources,
        evidenceLinks: uniqueStrings(evidence.map((item) => item.url), 8),
        discussionLinks,
        link: preferredLink,
...
export function createDatabase(databaseUrl) {
  ...
  async function attachBreakingLinks(breakingItemId, links) {
    for (const url of uniqueStrings(links.map(normalizeUrl).filter(Boolean), 12)) {
      await sql.query(
        `INSERT INTO breaking_item_links (url, breaking_item_id)
         VALUES ($1, $2)
         ON CONFLICT (url) DO UPDATE SET breaking_item_id = EXCLUDED.breaking_item_id`,
        [url, breakingItemId],
      );
    }
  }

  async function backfillBreakingLinks() {
    await sql.query(
      `INSERT INTO breaking_item_links (url, breaking_item_id)
       SELECT link, id FROM breaking_items
       ON CONFLICT (url) DO NOTHING`,
    );
  }

  async function collapseDuplicateLinks() {
    ...
    for (const { link } of groups) {
      const rows = await sql.query(
        `SELECT id, detected_at, source_count, sources, discussion_links
         FROM breaking_items
         WHERE link = $1
         ORDER BY detected_at DESC`,
        [link],
      );
      const merged = mergeDuplicateBreakingItems(rows);
      if (!merged?.removeIds.length) continue;
      await sql.query(
        `UPDATE breaking_items
         SET source_count = $2,
             sources = $3::jsonb,
             discussion_links = $4::jsonb,
             updated_at = NOW()
         WHERE id = $1`,
        [
          merged.keeperId,
          ...
          JSON.stringify(merged.discussionLinks),
        ],
      );
      const deleted = await sql.query(
        `DELETE FROM breaking_items
         WHERE link = $1 AND id <> $2
         RETURNING id`,
        [link, merged.keeperId],
      );
  ...
  async function mergeBreakingItemsByIds(ids) {
    const uniqueIds = uniqueStrings(ids, 20);
    if (uniqueIds.length < 2) return { keeperId: uniqueIds[0] ?? null, removed: 0 };
    const rows = await sql.query(
      `SELECT id, detected_at, source_count, sources, discussion_links, link
       FROM breaking_items
       WHERE id = ANY($1::text[])
       ORDER BY detected_at DESC`,
      [uniqueIds],
    ...
    const merged = mergeDuplicateBreakingItems(rows);
    if (!merged?.removeIds.length) {
      return { keeperId: merged?.keeperId ?? null, removed: 0 };
    }
    const aliases = uniqueStrings(
      [
        ...rows.map((row) => row.link),
        ...(await sql.query(
          `SELECT url FROM breaking_item_links WHERE breaking_item_id = ANY($1::text[])`,
          [uniqueIds],
        )).map((row) => row.url),
    ...
    const deleted = await sql.query(
      `DELETE FROM breaking_items
       ...
       RETURNING id`,
      [uniqueIds, merged.keeperId],
    );
    await attachBreakingLinks(merged.keeperId, aliases);
    return { keeperId: merged.keeperId, removed: deleted.length };
  }

  return {
    backfillBreakingLinks,
    collapseDuplicateLinks,
    mergeBreakingItemsByIds,
    ...
    async finishRun({ id, status, found, written, responseId, error = "" }) {
      await sql.query(
        `UPDATE radar_runs SET
           ...
           finished_at = NOW()
         WHERE id = $1`,
        [id, status, found, written, responseId, error || null],
      );
    },
    ...
    async recentContext() {
      const [breaking, briefings] = await Promise.all([
        sql.query(
          `SELECT id, title, detected_at, source_count, sources, link, updated_at
           FROM breaking_items
    ...
    async writeBreaking(topics, detectedAt) {
      await collapseDuplicateLinks();
      await backfillBreakingLinks();
      let written = 0;
      for (const topic of topics) {
        const canonicalLink = normalizeUrl(topic.link);
        const evidenceLinks = canonicalEvidenceLinks(topic);
        const aliasMatch = await sql.query(
          `SELECT bi.id, bi.detected_at, bi.source_count, bi.link
           FROM breaking_item_links bil
           JOIN breaking_items bi ON bi.id = bil.breaking_item_id
           WHERE bil.url = ANY($1::text[])
           ORDER BY bi.detected_at DESC
           LIMIT 1`,
          [evidenceLinks],
        );
        const previous = aliasMatch.length ? aliasMatch : await sql.query(
          `SELECT id, detected_at, source_count, link
           FROM breaking_items
           WHERE id = $1 OR link = $2
           ORDER BY CASE WHEN link = $2 THEN 0 ELSE 1 END, detected_at DESC
           LIMIT 1`,
          [topic.id, canonicalLink],
        );
        const existing = previous[0];
        const sameEvent = Boolean(existing);
        ...
        if (
          ...
          !(topic.materialUpdate && substantialSourceGrowth)
        ) {
          await attachBreakingLinks(existing.id, evidenceLinks);
          continue;
        }
        if (withinCooldown && !(topic.materialUpdate && substantialSourceGrowth)) {
          await attachBreakingLinks(existing.id, evidenceLinks);
          continue;
        }

        const stableId = sameEvent ? existing.id : topic.id;
        ...
        await attachBreakingLinks(stableId, evidenceLinks);
        written += 1;
      }
      await Promise.all([
